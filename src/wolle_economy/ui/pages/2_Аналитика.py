...
def tab_cashflow(df: pd.DataFrame) -> None:
    ...
    if len(avg_lag):
        ...
        fig.update_layout(height=300, margin={"l": 20, "r": 20, "t": 10, "b": 20}, showlegend=False)
        st.plotly_chart(fig, width="stretch")
...
def tab_trends(df: pd.DataFrame) -> None:
    ...
    pivot = hm.pivot_table(
        index="день_недели", columns="неделя", values="sell_price", aggfunc="sum", fill_value=0
    )
    days_order = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
    pivot = pivot.reindex([d for d in days_order if d in pivot.index])
    if not pivot.empty:
        fig = px.imshow(
            pivot, aspect="auto", color_continuous_scale="Viridis", labels={"color": "Выручка, ₽"}
        )
        fig.update_layout(height=320, margin={"l": 20, "r": 20, "t": 10, "b": 20})
        st.plotly_chart(fig, width="stretch")
...
def main() -> None:
    ...
    with tabs[1]:
        tab_abc(filtered)
    with tabs[2]:
        tab_returns(filtered)
    with tabs[3]:
        tab_suppliers(filtered)
    with tabs[4]:
        tab_pricing(filtered)
    with tabs[5]:
        tab_cashflow(filtered)
    with tabs[6]:
        tab_ops(filtered)
    with tabs[7]:
        tab_trends(filtered)
    with tabs[8]:
        tab_distribution(filtered)
